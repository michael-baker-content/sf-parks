...
export function ResultsMap({ destinations, styleUrl, preferCoreCity = false }: { destinations: MapDestination[]; styleUrl: string; preferCoreCity?: boolean }) {
  ...
  useEffect(() => {
    ...
    (async () => { try {
      ...
      map.on("load", () => {
        ...
        const data = { type: "FeatureCollection" as const, features: mappedDestinations.map((item) => ({ type: "Feature" as const, geometry: { type: "Point" as const, coordinates: [item.longitude, item.latitude] }, properties: { name: item.name, href: item.href } })) };
  ...
  return <div className="app-results-map" ref={container} />;
}
